...
const Login = () => {
	...
	const { user, loginUser, isLoading, signInWithGoogle, authLoginError, setAuthLoginError, authGoogleError, authSuccess, setAuthSuccess } = useFirebase();
...
{/* 
						...
						<input onBlur={handleOnBlur} type="email" placeholder="Your Email" required />
						<input onBlur={handleOnBlur} type="password" placeholder="Password" />

						<button className="my-2 loginBtn w-100" type="submit">Login</button>

						<div className="d-flex gap-5 justify-content-center align-items-center">
							<p>Not a member? <Link to="/register" className="text-decoration-none theme-primary-text fw-bold">Register</Link></p>
						</div>
					...
					<button className="google-sign-btn my-2 py-1" onClick={handleGoogleSignIn}>
